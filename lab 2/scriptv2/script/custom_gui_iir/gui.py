...
class gui():
    def __init__(self):
        ...
        self.d5 = Dock("Config")
        self.d6 = Dock("Zeros and Poles")
        self.d7 = Dock("Zeros and Poles")
        ...
        self.area.addDock(self.d7, 'left', self.d5)
        self.area.addDock(self.d6, 'left', self.d7)

        
        self.plot = pg.PlotWidget(title="Coeff b")
        self.plot.plotItem.showGrid(True,True)
        self.plot.addLegend()        
        pen1 = pg.mkPen(color=(255, 0, 0), width=2)
        pen2 = pg.mkPen(color=(0, 0, 255), width=2)
        self.figure_coeff1 = self.plot.plot([1,1,1,1,1,1],pen=pen1, symbol='o', symbolSize=5, symbolBrush=('r'),name='float - b')
        self.figure_coeff2 = self.plot.plot([1,1,1,1,1,1],pen=pen2, symbol='o', symbolSize=5, symbolBrush=('b'),name='fixed - b')
        self.d1.addWidget(self.plot)

        self.plot = pg.PlotWidget(title="Coeff a")
        self.plot.plotItem.showGrid(True,True)
        self.plot.addLegend()        
        pen1 = pg.mkPen(color=(255, 0, 0), width=2)
        pen2 = pg.mkPen(color=(0, 0, 255), width=2)
        self.figure_coeff3 = self.plot.plot([1,1,1,1,1,1],pen=pen1, symbol='o', symbolSize=5, symbolBrush=('r'),name='float - a')
        self.figure_coeff4 = self.plot.plot([1,1,1,1,1,1],pen=pen2, symbol='o', symbolSize=5, symbolBrush=('b'),name='fixed - a')
        self.d1.addWidget(self.plot)
...
#        self.figure_signals2 = self.plot.plot([2,2,2,2,2,2],pen=pen2, symbol='o', symbolSize=10, symbolBrush=('b'))
        ...
        self.plot = pg.PlotWidget(title="Floating Point")
        self.plot.addLegend()        
        self.plot.plotItem.showGrid(True,True)
        r=1
        n=100
        ...
        for x in range(0,n+1):
            xcir.append((math.cos(2*math.pi/n*x)*r))
            ycir.append((math.sin(2*math.pi/n*x)*r))
        self.figure_circle = self.plot.plot(xcir,ycir)        
        self.figure_poles1 = self.plot.plot([1,1,1,1,1,1],pen=None, symbol='x', symbolSize=10, symbolBrush=('r'),name='poles')        
        self.figure_zeros1 = self.plot.plot([1,1,1,1,1,1],pen=None, symbol='o', symbolSize=10, symbolBrush=('b'),name='zeros')
        self.d7.addWidget(self.plot)

        self.plot = pg.PlotWidget(title="Fixed Point")
        self.plot.addLegend()        
        self.plot.plotItem.showGrid(True,True)
        r=1
        n=100
        ...
        for x in range(0,n+1):
            ...
            ycir.append((math.sin(2*math.pi/n*x)*r))
        self.figure_circle = self.plot.plot(xcir,ycir)        
        self.figure_poles2 = self.plot.plot([1,1,1,1,1,1],pen=None, symbol='x', symbolSize=10, symbolBrush=('r'),name='poles')        
        self.figure_zeros2 = self.plot.plot([1,1,1,1,1,1],pen=None, symbol='o', symbolSize=10, symbolBrush=('b'),name='zeros')
        self.d6.addWidget(self.plot)
        ...
        self.w1 = Slider('NBT_Coeff_a',4,1, 32)
        self.QGLRoot.addWidget(self.w1)
        self.w2 = Slider('NBF_Coeff_a',2,1, 32)
        self.QGLRoot.addWidget(self.w2)
 
        self.w3 = Slider('NBT_Coeff_b',4,1, 32)
        self.QGLRoot.addWidget(self.w3)
        self.w4 = Slider('NBF_Coeff_b',2,1, 32)
...
#        self.QGLRoot.addWidget(self.w5)
 ...
 #       self.w6 = Slider('cutoff',6000,1, 20000)
  #      self.QGLRoot.addWidget(self.w6)
        ...
        self.w1.slider.valueChanged.connect(self.update_maxNBF2)
        # self.w2.slider.valueChanged.connect(self.update_plot)
        self.w3.slider.valueChanged.connect(self.update_maxNBF4)
        # self.w4.slider.valueChanged.connect(self.update_plot)

    def update_maxNBF2(self):        
        self.w2.slider.setMaximum(self.w1.slider.value())

    def update_maxNBF4(self):        
        self.w4.slider.setMaximum(self.w3.slider.value())
    ...
    def plot_coeff(self, data1,data2, data3,data4):
        self.figure_coeff1.setData(data1)
        self.figure_coeff2.setData(data2)
        self.figure_coeff3.setData(data3)
        self.figure_coeff4.setData(data4)

    def plot_poleszeros(self, poles,zeros,option=True):
        if option:
            self.figure_poles1.setData(poles.real,poles.imag)
            self.figure_zeros1.setData(zeros.real,zeros.imag)
        else:
            self.figure_poles2.setData(poles.real,poles.imag)
            self.figure_zeros2.setData(zeros.real,zeros.imag)
